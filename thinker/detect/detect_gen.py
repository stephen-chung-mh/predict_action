...
class DetectBuffer:
    def __init__(self, outdir, t, rec_t, logger, delay_n=5, total_n=70000):
        """
        ...
        self.file_names = np.arange(total_n)
    ...
    def insert(self, xs, ys, done, step_status):
        ...
        if (self.processed_n >= self.t + self.delay_n + 1):               
            ...
            self.logger.info(f"{self.file_idx}: File saved to {self.outdir}; env_state shape {out_shape}")

        return self.file_idx >= self.total_n
    ...
    def _save(self, out):        
        ...
        for n in range(N):
            for b in range(B):
                if self.file_idx >= self.total_n: break
                path = f'{self.outdir}/data_{self.file_names[self.file_idx]}.pt'        
                save_x = {}
                for k, v in x.items():
                    if k == "done":
                        save_x[k] = v[n, b]
                        ...
                        save_x[k] = torch.clone(v[n, :, b]) if v is not None else None
                save_y = {k:v[n, b] if v is not None else None for k, v in y.items()}
                save_out = (save_x, save_y)
                torch.save(save_out, path)
                self.file_idx += 1
...
def conv_greyscale(x):
    B, C3, H, W = x.shape
    C = C3 // 3
    # Reshape x to (B, C, 3, H, W)
    x = x.view(B, C, 3, H, W)    
    ...
    y = (x * weights.view(1, 1, 3, 1, 1)).sum(dim=2)    
    return y


def detect_gen(total_n, env_n, delay_n, mcts_k, greedy, greyscale, skip_im, savedir, outdir, xpid, suffix="", n=0):
    ...
    while True:
        ...
        if not os.path.exists(outdir_):
            os.makedirs(outdir_)
            os.makedirs(os.path.join(outdir_, "train"))
            os.makedirs(os.path.join(outdir_, "val"))
            os.makedirs(os.path.join(outdir_, "test"))
    ...
    if skip_im: 
        rec_t = 1
    elif not im_rollout and not mcts:
        rec_t = flags.rec_t 
    elif mcts:
        rec_t = delay_n * mcts_k + 1
    else:
        rec_t = delay_n + 1
    
    #detect_buffer = DetectBuffer(outdir=outdir, t=12800//env_n, rec_t=rec_t, logger=_logger, delay_n=delay_n, total_n=total_n)
    train_detect_buffer = DetectBuffer(outdir=os.path.join(outdir, "train"), t=20, rec_t=rec_t, logger=_logger, delay_n=delay_n, total_n=total_n)
    val_detect_buffer = DetectBuffer(outdir=os.path.join(outdir, "val"), t=20, rec_t=rec_t, logger=_logger, delay_n=delay_n, total_n=10000)
    test_detect_buffer = DetectBuffer(outdir=os.path.join(outdir, "test"), t=20, rec_t=rec_t, logger=_logger, delay_n=delay_n, total_n=10000)
    _logger.info(f"Data output directory: {outdir}")

    detect_buffer_ls = [train_detect_buffer, val_detect_buffer, test_detect_buffer]

    with torch.set_grad_enabled(False):
        for dm, detect_buffer in enumerate(detect_buffer_ls):
            state = env.reset(reset_stat=True)
            ...
            if dm == 0:
                ...
                if greyscale: 
                    env_state_shape = (env_state_shape[0] // 3,) + (env_state_shape[1:])
                ...
                flags_detect = {
                    ...
                    "tree_rep_shape": list(tree_rep_shape) if not disable_thinker and not mcts else None,
            ...
            while(True):
                ...
                if last_step_real:
                    costs.append(torch.mean(info['cost'].float()).item())
                ...
                if im_rollout and len(last_im_actions_buffer) == delay_n:
                    mask = torch.zeros(env_n, dtype=torch.bool, device=device)
                    for t in range(delay_n):
                        acc = torch.all(last_real_actions[:, :t+1] == last_im_actions_buffer[0][:, :t+1], dim=-1)
                        num = torch.sum(1 - mask.float())
                        if num > 0:
                            acc = (torch.sum(acc.float() * (1 - mask.float())) / num).item()
                            accs[t].append(acc)                        
                        if t < delay_n - 1: mask = torch.logical_or(mask, last_dones[:, t+1])
                ...
                if greyscale: env_state = conv_greyscale(env_state)
                xs = {
                    "env_state": env_state,
                ...
                if not disable_thinker:
                    ...
                    if disable_thinker:
                        xs.update({
                            "hidden_state": actor_net.hidden_state.half()
                        })                          
                ...
                if not (mcts and step_status != 0) and not (skip_im and not last_step_real): # no recording for non-real mcts
                    ...
                    buffer_done = detect_buffer.insert(xs, ys, done, step_status)
                ...
                if im_rollout or (mcts and next_step_real):
                    ...
                    if mcts:
                        most_visited_actions = torch.tensor(env.most_visited_path(delay_n, k=mcts_k), dtype=torch.long, device=device)                    
                    unroll_n = 1 if not mcts else mcts_k                    
                    last_im_actions = []
                    for k in range(unroll_n):
                        ...
                        for m in range(delay_n):
                            if not mcts:
                                actor_out_, im_actor_state = actor_net(env_out=new_env_out, core_state=im_actor_state, greedy=greedy)   
                                action = actor_out_.action
                            else:
                                action = most_visited_actions[k, m]
                            if k == 0: last_im_actions.append(action)
                            ...
                            env_state_ = xs[0].half()
                            if greyscale: env_state_ = conv_greyscale(env_state_)
                            xs = {
                                "env_state": env_state_,
                                "pri_action": action,            
                                "cost": torch.zeros_like(info["cost"]),
                            }
                            if im_rollout: xs["hidden_state"] = actor_net.hidden_state.half()
                            if mcts: xs["reset_action"] = torch.zeros_like(actor_out.action[1])
                            buffer_done = detect_buffer.insert(
                                xs, 
                                None, 
                                torch.zeros_like(done), 
                                2 if m == delay_n - 1 and k == unroll_n - 1 else 1
                            )
                    last_im_actions = torch.stack(last_im_actions, dim=-1)
                    last_im_actions_buffer.append(last_im_actions)
                
                if buffer_done: break   

                if print_n % 100 == 0:
                    r = np.mean(np.array(rets)) if len(rets) > 0 else 0.
                    print_str = f"{print_n} - Episode {len(rets)}; Return {r:2f}"
                    if len(accs) > 0 and len(accs[0]) > 0: 
                        print_str += f" im-rollout acc "
                        for t in range(delay_n):
                            print_str += f"{np.mean(np.array(accs[t])):.5f} "
                    print_str += f" mean_cost {np.mean(np.array(costs)):.5f}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"Thinker data generation")
    ...
    parser.add_argument("--total_n", default=50000, type=int, help="Number of real steps.")
    parser.add_argument("--env_n", default=64, type=int, help="Batch size in generation.")
    parser.add_argument("--delay_n", default=5, type=int, help="Delay step in predicting danger.")
    parser.add_argument("--no_greedy", action="store_true", help="Dsiable greedy policy.")
    ...
    parser.add_argument("--no_greyscale", action="store_true", help="Don't convert to greyscale.")
    parser.add_argument("--mcts_k", default=1, type=int, help="Numer of mcts rollouts.")
    ...
    detect_gen(
        ...
        mcts_k=flags.mcts_k,
        greedy=not flags.no_greedy,         
        greyscale=not flags.no_greyscale,
