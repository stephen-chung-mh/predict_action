...
cdef class cWrapper():
    ...
    def most_visited_path(self, n, k=1):
        cdef vector[Node*] nodes, new_nodes
        cdef Node node, child
        cdef Node* ptop_child
        cdef int i, j, m, l, u, most_visited, tmp_vc, raw_vc, vc

        actions = np.zeros((k, n, self.env_n), dtype=np.intc)
        vcs = np.zeros((k, self.env_n), dtype=np.intc)        

        for l in range(k):
            nodes = self.root_nodes
            for m in range(n):            
                for i in range(self.env_n):
                    most_visited, tmp_vc, raw_vc, vc = 0, 0, 0, 0
                    node = nodes[i][0]
                    ptop_child = nodes[i]
                    for j in range(int(node.ppchildren[0].size())):
                        child = node.ppchildren[0][j][0]
                        tmp_vc = child.rollout_n
                        for u in range(l):
                            if m == 0 or np.all(actions[u, :m, i] == actions[l, :m, i]):
                                if j == actions[u, m, i]:
                                    tmp_vc = tmp_vc - vcs[u, m] # this simply remove the vc of the already recorded action seq
                        if tmp_vc > vc:
                            most_visited = j
                            vc = tmp_vc
                            raw_vc = child.rollout_n
                            ptop_child = node.ppchildren[0][j]
                    new_nodes.push_back(ptop_child)
                    actions[l, m, i] = most_visited
                    if m == n - 1: vcs[l, i] = raw_vc
                nodes = new_nodes
                new_nodes.clear()                
        return actions
